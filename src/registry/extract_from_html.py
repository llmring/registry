        ...
        if match:
            try:
                ...
                if model_id == 'gpt-5':
                    ...
                    context = 128000
                    description = "Fastest, cheapest for summarization and classification"
        ...
        if match:
            try:
                ...
                models.append({
                    ...
                    'supports_vision': 'gpt-4o' in model_id or 'gpt-4-turbo' in model_id,
                    'supports_function_calling': True,
                    'supports_json_mode': True,
                    'supports_parallel_tool_calls': True,
                })
...
def extract_anthropic_models(html: str) -> List[Dict[str, Any]]:
        ...
        if match:
            try:
                ...
                if input_price > 1000 or output_price > 1000:
                    ...
                    logger.warning(f"Suspicious price for {model_id}: ${input_price}/$M input, ${output_price}/$M output")
    ...
    if not models:
        logger.warning(f"No Anthropic models could be extracted from HTML - regex patterns may need updating")
...
def extract_google_models(html: str) -> List[Dict[str, Any]]:
    ...
    model_patterns = [
        (r'gemini-1\.5-pro.*?\$?([\d.]+).*?input.*?\$?([\d.]+).*?output', 'gemini-1.5-pro'),
        (r'gemini-1\.5-flash(?!-8b).*?\$?([\d.]+).*?input.*?\$?([\d.]+).*?output', 'gemini-1.5-flash'),
        (r'gemini-1\.5-flash-8b.*?\$?([\d.]+).*?input.*?\$?([\d.]+).*?output', 'gemini-1.5-flash-8b'),
    ...
    for new_model in new_models:
        ...
        if model_id in existing_models:
            ...
            existing_models[model_id].update({
                'dollars_per_million_tokens_input': new_model.get('dollars_per_million_tokens_input'),
                'dollars_per_million_tokens_output': new_model.get('dollars_per_million_tokens_output'),
        ...
        if all_models:
            ...
            for model in all_models:
                ...
                if model_id and model_id not in unique_models:
                    ...
                    if output_price > 500:
                        issues.append(f'suspiciously high output price: ${output_price}')
