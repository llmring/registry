...
def compare_json_models(old_data: Dict, new_data: Dict) -> bool:
    ...
    for old, new in zip(old_sorted, new_sorted):
        ...
        for field in ["model_id", "max_context", "max_output_tokens",
                     "dollars_per_million_tokens_input", 
                     "dollars_per_million_tokens_output",
                     "supports_vision", "supports_function_calling"]:
            if old.get(field) != new.get(field):
                logger.info(f"Difference found in {field}: {old.get(field)} vs {new.get(field)}")
                return True
...
@click.option('--models-dir', default='models', help='Directory for current model JSONs')
